...
EXEMPT_ROLE_NAME = "odb-developer-role"
...
class ODBCreateActionCheck(BaseResourceCheck):
    def __init__(self):
        super().__init__(
            name="Do not allow odb:create* actions in IAM policies",
    ...
    def _all_roles_exempt(self, roles: list, template_resources: dict) -> bool:
        ...
        for role in roles:
            if isinstance(role, dict) and "Ref" in role:
                ...
                dbg(f"Ref={logical_id} → RoleName={role_name} (expected={EXEMPT_ROLE_NAME})")
                if role_name != EXEMPT_ROLE_NAME:
